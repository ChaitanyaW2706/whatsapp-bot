...
class WhatsAppBot:
    ...
    def process_message(self, phone_number: str, message: str):
        ...
        # ===== BUTTON HANDLERS =====
        if message in ('book_appointment', 'book_appointment'):
            return self.book_service_appointment(phone_number)

        if message in ('contact_advisor', 'talk_to_advisor'):
        ...
        if message in ('service_estimate', 'get_estimate'):
            if not session.get('vehicle_reg'):
